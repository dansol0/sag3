class usershowPdf < Prawn::Document

 
 def initialize(user, view, user)
  ...
  def body
  		...
  		text "ACTIVIDAD ID:  \##{@user.id}", size: 13, style: :bold, :color =>"0000FF", align: :center
			 move_down 8  
		  data = [
		          		 ["TIPO", "#{@user.tipo}"],
						 ["TITULO", "#{@user.titulo}"],
						 ...
						 ["HORAS EMPLEADAS", "#{@user.horas}"],
						 ["FECHA INICIO", "#{@user.fecha_i}"],
						 ["FECHA DE CULMINACIÓN", "#{@user.fecha_f}"],
						 ["PERIODO", "#{@user.ano_periodo}"]
						 ]
