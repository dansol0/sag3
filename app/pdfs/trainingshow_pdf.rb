class TrainingshowPdf < Prawn::Document

 
 def initialize(training, view, user)
  ...
  def header (title=nil)
  	 text "ACTIVIDAD DEL ÁREA DE MEJORAMIENTO Y CAPACITACIÓN", size: 15, style: :bold, align: :center
    move_down 30
    bounding_box([25, cursor - 0], :width => 400, :height => 100) do
    	move_down 10
    	span(500,:position =>:center)do
	    	text "Mejoramiento y capacitación", style: :bold, :size => 12
	    	move_down 10
  ...
  def body
  		...
  		text "ACTIVIDAD ID:  \##{@training.id}", size: 13, style: :bold, :color =>"0000FF", align: :center
			 move_down 8  
		  data = [
		          		 ["TIPO", "#{@training.tipo}"],
						 ["NOMBRE", "#{@training.nombre}"],
						 ["DESCRIPCIÓN", "#{@training.descripcion}"],
						 ["HORAS EMPLEADAS", "#{@training.horas}"],
						 ["FECHA INICIO", "#{@training.fecha_i}"],
						 ["FECHA DE CULMINACIÓN", "#{@training.fecha_f}"],
						 ["PERIODO", "#{@training.ano_periodo}"]
						 ]
