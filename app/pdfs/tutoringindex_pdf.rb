class TutoringindexPdf < Prawn::Document

 
 def initialize(tutorings, view)
  ...
  def header (title=nil)
    ...
    bounding_box([25, cursor - 0], :width => 320, :height => 65) do
    	...
    	span(500,:position =>:center)do
	    	text "Lista de tutorías del área de docencia ", style: :bold, :size => 12
	    	move_down 10
 ...
 def item_header
  ...
  def item_rows
  	...
  	@tutorings.each.map do |tutoring|
  	 [tutoring.id, tutoring.tipo, tutoring.nombre, tutoring.fecha_i] 
  	end
  end
