class TablaExtensionOtraPdf < Prawn::Document

 
 def initialize(extensions, nombre, view)
  ...
  def header (title=nil)
  	 text "LISTA DE ACTIVIDADES DEL DEPARTAMENTO DE EXTENSIÓN" , size: 15, style: :bold, align: :center
    move_down 10
    text  @nombre, size: 15, style: :bold, align: :center
    move_down 30
 ...
 def item_header
  ...
  def item_rows
  	...
  	@extensions.each.map do |extension|
  			#aux = User.where(id: subject.creador)
  	 [extension.descripcion, extension.nombre] 
  	end
