class ResearchshowPdf < Prawn::Document

 
 def initialize(research, view, user)
  ...
  def header (title=nil)
    ...
    bounding_box([25, cursor - 0], :width => 400, :height => 100) do
    	...
    	span(500,:position =>:center)do
	    	text "Investigación", style: :bold, :size => 12
	    	move_down 10
  ...
  def body
  		...
  		text "ACTIVIDAD ID:  \##{@research.id}", size: 13, style: :bold, :color =>"0000FF", align: :center
			 move_down 8  
		  data = [
		          		 ["TIPO", "#{@research.tipo}"],
						 ["NOMBRE", "#{@research.nombre}"],
						 ["DESCRIPCIÓN", "#{@research.descripcion}"],
						 ["HORAS EMPLEADAS", "#{@research.horas}"],
						 ["FECHA INICIO", "#{@research.fecha_i}"],
						 ["FECHA DE CULMINACIÓN", "#{@research.fecha_f}"],
						 ["PERIODO", "#{@research.ano_periodo}"]
						 ]
