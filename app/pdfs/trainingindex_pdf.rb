class TrainingindexPdf < Prawn::Document

 
 def initialize(trainings, view)
  ...
  def header (title=nil)
  	 text "ACTIVIDADES DE MEJORAMIENTO Y CAPACITACIÓN", size: 15, style: :bold, align: :center
    move_down 30
    bounding_box([25, cursor - 0], :width => 320, :height => 65) do
    	move_down 10
    	span(500,:position =>:center)do
	    	text "Lista de actividades de mejoramiento y capacitación ", style: :bold, :size => 12
	    	move_down 10
 ...
 def item_header
  ...
  def item_rows
  	...
  	@trainings.each.map do |training|
  	 [training.id, training.tipo, training.nombre, training.fecha_i] 
  	end
  end
