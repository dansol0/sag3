class PublicationindexPdf < Prawn::Document

 
 def initialize(publications, view)
  ...
  def header (title=nil)
  	 text "ACTIVIDADES DEL ÁREA DE INVESTIGACIÓN", size: 15, style: :bold, align: :center
    move_down 30
    bounding_box([25, cursor - 0], :width => 300, :height => 65) do
    	move_down 10
    	span(500,:position =>:center)do
	    	text "Lista de publicaciones del área de investigación ", style: :bold, :size => 12
	    	move_down 10
 ...
 def item_header
 	["ID","TITULO","REVISTA O EDITORIAL","FECHA INICIO"]

 end

  def item_rows
                  
  	@publications.each.map do |publication|
  	 [publication.id, publication.titulo, publication.revista_editorial, publication.fecha_i] 
  	end
  end
