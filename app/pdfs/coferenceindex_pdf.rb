class CoferenceindexPdf < Prawn::Document

 
 def initialize(coferences, view)
	  super(:margin => [15,15,15,15])
	  @coferences = coferences
	   @view = view
  ...
  def header (title=nil)
  	 text "ACTIVIDADES DEL ÁREA DE EXTENSIÓN", size: 15, style: :bold, align: :center
    move_down 30
    bounding_box([25, cursor - 0], :width => 300, :height => 65) do
    	move_down 10
    	span(500,:position =>:center)do
	    	text "Lista de conferencias o ponencias en eventos ", style: :bold, :size => 12
	    	text "nacionales o internacionales", style: :bold, :size => 12
	    	move_down 10
 ...
 def item_header
 	["ID","TIPO","TITULO", "EVENTO","FECHA INICIO"]

 end

  def item_rows
                  
  	@coferences.each.map do |coference|
  	 [coference.id, coference.tipo, coference.titulo, coference.evento, coference.fecha_i] 
  	end
  end
