class ExtensionindexPdf < Prawn::Document

 
 def initialize(extensions, view)
  ...
  def header (title=nil)
    ...
    bounding_box([25, cursor - 0], :width => 300, :height => 65) do
    	...
    	span(500,:position =>:center)do
	    	text "Lista de otras actividades del área de extensión ", style: :bold, :size => 12
	    	move_down 10
 ...
 def item_header
  ...
  def item_rows
  	...
  	@extensions.each.map do |extension|
  	 [extension.id, extension.tipo, extension.nombre, extension.fecha_i] 
  	end
  end
