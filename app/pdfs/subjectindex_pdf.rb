class SubjectindexPdf < Prawn::Document

 
 def initialize(subjects, view)
  ...
  def header (title=nil)
    ...
    move_down 30
    bounding_box([25, cursor - 0], :width => 280, :height => 75) do
    	move_down 10
    	span(500,:position =>:center)do
	    	text "Lista de dictado de asignaturas del área", style: :bold, :size => 12
	    	move_down 10
	    	text " de investigación ", style: :bold, :size => 12
	    	move_down 10
 ...
 def item_header
  ...
  def item_rows
  	...
  	@subjects.each.map do |subject|
  	 [subject.id, subject.tipo, subject.nombre, subject.fecha_i] 
  	end
  end
