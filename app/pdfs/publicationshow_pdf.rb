class PublicationshowPdf < Prawn::Document

 
 def initialize(publication, view, user)
    ...
    move_down 30
     bounding_box([25, cursor - 0], :width => 400, :height => 100) do
    	...
    	span(500,:position =>:center)do
	    	text "Publicación", style: :bold, :size => 12
	    	move_down 10
  ...
  def body
  		...
  		text "ACTIVIDAD ID:  \##{@publication.id}", size: 13, style: :bold, :color =>"0000FF", align: :center
			 move_down 8  
		  data = [
		          		 ["TITULO", "#{@publication.titulo}"],
						 ["REVISTA O EDITORIAL", "#{@publication.revista_editorial}"],
						 ["ARBITRADA O NO ARBITRADA", "#{@publication.arbitr_no_arbitr}"],
						 ["DESCRIPCIÓN", "#{@publication.descripcion}"],
						 ["HORAS EMPLEADAS", "#{@publication.horas}"],
						 ["FECHA INICIO", "#{@publication.fecha_i}"],
						 ["FECHA DE CULMINACIÓN", "#{@publication.fecha_f}"],
						 ["PERIODO", "#{@publication.ano_periodo}"]
						 ]
