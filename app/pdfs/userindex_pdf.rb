class UserindexPdf < Prawn::Document
 ...
 def initialize(users, view)
  ...
  def header (title=nil)
  	 text "LISTA DE INVESTIGADORES", size: 15, style: :bold, align: :center
    move_down 30
 ...
 def item_header
 	["CEDULA","PRIMER NOMBRE","PRIMER APELLIDO","EMAIL", "ROL"]
  ...
  def item_rows
  	...
  	@users.each.map do |user |
  	 [user.id, user.nombre1, user.apellido1, user.email, user.rol] 
  	end
  end
