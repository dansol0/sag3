class WorkindexPdf < Prawn::Document

 
 def initialize(works, view)
  ...
  def header (title=nil)
    ...
    move_down 30
    bounding_box([25, cursor - 0], :width => 400, :height => 75) do
    	move_down 10
    	span(500,:position =>:center)do
	    	text "Lista de actividades de jurado de trabajos de grado, postgrado y ", style: :bold, :size => 12
	    	move_down 10
	    	text " ascensos, arbitraje o evaluación de cursos del área de docencia ", style: :bold, :size => 12
	    	move_down 10
 ...
 def item_header
 	["ID","TIPO","TITULO","FECHA INICIO"]

 end

  def item_rows
                  
  	@works.each.map do |work|
  	 [work.id, work.tipo, work.titulo, work.fecha_i] 
  	end
  end
