class ResearchindexPdf < Prawn::Document

 
 def initialize(researches, view)
  ...
  def header (title=nil)
    ...
    bounding_box([25, cursor - 0], :width => 320, :height => 65) do
    	...
    	span(500,:position =>:center)do
	    	text "Lista de otras actividades del área de investigación ", style: :bold, :size => 12
	    	move_down 10
 ...
 def item_header
  ...
  def item_rows
  	...
  	@researches.each.map do |research|
  	 [research.id, research.tipo, research.nombre, research.fecha_i] 
  	end
  end
