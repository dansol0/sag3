class CoferenceshowPdf < Prawn::Document

 
 def initialize(coference, view, user)
  ...
  def header (title=nil)
    ...
    bounding_box([25, cursor - 0], :width => 400, :height => 100) do
    	...
    	span(500,:position =>:center)do
	    	text "Conferencias o ponencias en eventos nacionales o internacionales", style: :bold, :size => 12
	    	move_down 10
  ...
  def body
  		...
  		text "ACTIVIDAD ID:  \##{@coference.id}", size: 13, style: :bold, :color =>"0000FF", align: :center
			 move_down 8  
		  data = [
		          		 ["TIPO", "#{@coference.tipo}"],
						 ["TITULO", "#{@coference.titulo}"],
						 ["EVENTO", "#{@coference.evento}"],
						 ["DESCRIPCIÓN", "#{@coference.descripcion}"],
						 ["HORAS EMPLEADAS", "#{@coference.horas}"],
						 ["FECHA INICIO", "#{@coference.fecha_i}"],
						 ["FECHA DE CULMINACIÓN", "#{@coference.fecha_f}"],
						 ["PERIODO", "#{@coference.ano_periodo}"]
						 ]
