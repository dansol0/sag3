class ActivityshowPdf < Prawn::Document

 
 def initialize(activity, view, user)
  ...
  def header (title=nil)
  	 text "ACTIVIDAD DEL ÁREA ACADÉMICO ADMINISTRATIVA", size: 15, style: :bold, align: :center
    move_down 30
    bounding_box([25, cursor - 0], :width => 400, :height => 100) do
    	move_down 10
    	span(500,:position =>:center)do
	    	text "Académico administrativa", style: :bold, :size => 12
	    	move_down 10
  ...
  def body
  		...
  		text "ACTIVIDAD ID:  \##{@activity.id}", size: 13, style: :bold, :color =>"0000FF", align: :center
			 move_down 8  
		  data = [
		          		 ["TIPO", "#{@activity.tipo}"],
						 ["NOMBRE", "#{@activity.nombre}"],
						 ["DESCRIPCIÓN", "#{@activity.descripcion}"],
						 ["HORAS EMPLEADAS", "#{@activity.horas}"],
						 ["FECHA INICIO", "#{@activity.fecha_i}"],
						 ["FECHA DE CULMINACIÓN", "#{@activity.fecha_f}"],
						 ["PERIODO", "#{@activity.ano_periodo}"]
						 ]
