class ProyectindexPdf < Prawn::Document
  ...
  def header (title=nil)
    ...
    bounding_box([25, cursor - 0], :width => 300, :height => 65) do
    	...
    	span(500,:position =>:center)do
	    	text "Lista de proyectos del área de investigación ", style: :bold, :size => 12
	    	move_down 10
 ...
 def item_header
 	["ID","TITULO","ESTATUS","FECHA INICIO"]
  ...
  def item_rows
  	...
  	@proyects.each.map do |proyect|
  	 [proyect.id, proyect.titulo, proyect.estatus, proyect.fecha_i] 
  	end
  end
